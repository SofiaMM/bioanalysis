package com.techprimers.db.utils;
...
import com.techprimers.db.repository.ExternalSourceRepository;
...
//			name = nrepo.save(name);
			...
			List <MetaboliteName> nms = nrepo.findByName(n);
			if (nms.size() == 0) {
				name = nrepo.save(name);
			} else {
				name = nms.get(0);
		...
		Map<String, Set<String>> ext = m.getMapSourceExternalIdSet();
		for(String id : ext.keySet()) {
			
			List <ExternalSource> exts = extrepo.findByName(id);
			ExternalSource extsource = new ExternalSource();
			if(exts.size() > 0) {
				extsource = exts.get(0);
			} else {
				extsource.setName(id);
				extsource = extrepo.save(extsource);
			}
			
			for(String extName : ext.get(id)) {
				ExternalSourceId extId = new ExternalSourceId();
				extId.setExternalSource(extsource);
				extId.setMetabolite(met);
				
				List <ExternalSourceId> ids = idrepo.findByName(extName);
				if (ids.size()==0) {
					extId = idrepo.save(extId);
				} else {
					extId.setName(extName);
					extId = ids.get(0);
				}
			}
		}
	...
	protected BioanalysisMetabolitePrice convertPrice(BioeconomicsMetabolitePrice p) {
		...
		List<Provider> op = provrepo.findByName(provider.getName());  //Criar uma lista com todos os nomes dos providers
